...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const currentCounts = useRef<Record<string, number>>({
  ...
  useEffect(() => {
    ...
    if (pendingCount !== prev.pending) {
      changed.pending = now;
      unmarkNotificationRead("pending");
    }
    if (dueCount !== prev.due) {
      changed.due = now;
      unmarkNotificationRead("due");
    }
    if (readyCount !== prev.ready) {
      changed.ready = now;
      unmarkNotificationRead("ready");
    }
  ...
  return (
    <SidebarProvider>
      <div className="min-h-screen flex w-full">
        ...
        <div className="flex-1 flex flex-col h-screen overflow-x-hidden overflow-y-auto">
          <header
            className="h-12 md:h-14 flex items-center justify-between bg-card/80 backdrop-blur-xl px-3 md:px-4 shrink-0 sticky top-0 z-30 shadow-sm"
            style={{
              borderBottom: "1px solid hsl(var(--border))",
              boxShadow:
                "0 1px 0 hsl(var(--primary) / 0.08), 0 2px 8px hsl(var(--background) / 0.6)",
            }}
          >
                      ...
                      <div className="py-1">
                        {unreadNotifications.map((n) => (
                          ...
                          >
                            ...
                            <span className="text-xs font-bold bg-muted px-2 py-0.5 rounded-full shrink-0">
                              {n.count}
                            </span>
                          </button>
                        ))}
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem
                    ...
                    className={language === "bn" ? "bg-accent/20" : ""}
                  >
                    🇧🇩 {t("bangla")}
                  </DropdownMenuItem>
                  <DropdownMenuItem
